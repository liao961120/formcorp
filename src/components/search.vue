    ...
    <div v-scroll="handleScroll">
        <div class="results" :key="search_results_key">
            <template v-for="(res, i) in vue_seach_results_lazy">
                <Leipzig v-bind:gloss="res" v-bind:query="query" v-bind:showplaintext="false" v-bind:ignoreCharSet="ignoreCharSet" :key="i" />
            </template>
        </div>
                        ...
                        if (this.query.regex == 0) {
                            let isMatch = gloss_content.some(
                                ...
                                tk => tk.includes(search_pats[j]) |
                                      (this.normToken(tk).includes(norm_pat) & norm_pat.length > 1)
                            );
                            if (isMatch) matchNum++;
                        // Regex search
                        } else {
